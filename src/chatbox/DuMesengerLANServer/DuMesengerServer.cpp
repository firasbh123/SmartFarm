...
{
    qDebug() << "Server : connected with handle : " << handle;
    ...
    for(auto i : mSockets){
        QTextStream T(i);
        T << "Server : Connextion detecter :" << handle;
        i->flush();
    }
    connect(socket, &DuMesengerSocket::DuReadyRead, [&](DuMesengerSocket *S){
        qDebug() << "Nouveau Message envoyee !";
        QTextStream T(S);
        auto text = T.readAll();
        for(auto i : mSockets)
        {
    ...
    connect(socket, &DuMesengerSocket::DuStateChanged,
        ...
        qDebug() << "Server : DuStateChanged with handle : "
                 << S->socketDescriptor();

        if (ST == QTcpSocket::UnconnectedState)
        {
            qDebug() << "Server : Unconnected State with handle "
                     << S->socketDescriptor();
